...
static inline fdt_addr_t rockchip_dev_read_addr_size_prop(struct udevice *dev, const char *property, fdt_size_t *sizep)
{
...
static int rockchip_vop_init_gamma(struct vop *vop, struct display_state *state)
{
	...
	lut_regs = (u32 *)rockchip_dev_read_addr_size_prop(crtc_state->dev, "reg", &lut_size);
...
static int rockchip_vop_init(struct display_state *state)
{
	...
	ret = clk_set_defaults(crtc_state->dev, 1); /* (SS) NB: is "1" correct? */
	if (ret)
