...
#include "supp_headers/linux/hdmi.h"
#include <linux/list.h>
#include <linux/compat.h>
#include "supp_headers/linux/media-bus-format.h"
#include <malloc.h>
#include <video.h>
#include "supp_headers/video_rockchip.h"
...
#define SCREEN_MODE_CONSOLE_ACTIVE 0
#define SCREEN_MODE_LOGO_ACTIVE 1

static unsigned long console_fbbase;
...
/*
 * A method to dump the state of the hardware memory-mapped
 * registers for debugging.
 */
void debug_binary_dump_grf(const char *caption, void *origin, void *end)
{
#ifdef DEBUG
	int i, nbit;
	u32 *cur_addr = (u32 *)origin;
	u32 reg_word;
	
	printf("%s:\n", caption);
	
	while (cur_addr < (u32 *)end) {
		printf("%016lX: ", (unsigned long)cur_addr);
		reg_word = *(cur_addr++);
...
int rockchip_get_baseparameter(void)
{
	...
	ret = rockchip_get_bootdev_desc(&dev_desc);
	if (!dev_desc) {
...
static int display_init(struct display_state *state)
{
	...
	if (panel_state->panel) {
		...
		if (ret > 0) {
			...
			ret = rockchip_bridge_get_timing(conn_state->bridge);
		}
...
static int display_crtc_purge(struct display_state *state)
{
	struct crtc_state *crtc_state = &state->crtc_state;
...
static int display_disable(struct display_state *state, int do_deinit)
{
	struct connector_state *conn_state = &state->conn_state;
	const struct rockchip_connector *conn = conn_state->connector;
	const struct rockchip_connector_funcs *conn_funcs = conn->funcs;
...
#ifdef CONFIG_DRM_ROCKCHIP_VIDEO_FRAMEBUFFER
static int display_shutdown(struct display_state *state)
{
	display_crtc_purge(state);
	display_disable(state, 1);
...
static int display_logo(struct display_state *state)
{
	...
	debug("(SS) 'logo' size: %d x %d\n", logo->width, logo->height);
	debug("(SS) 'logo' bpp: %d\n", logo->bpp);
	debug("(SS) 'logo' offset: %08X\n", logo->offset);
	...
	/* force a test pattern in */
	for(unsigned int i = 0; i < (logo->width*logo->height*logo->bpp)/8; ++i) {
		*(((char *)crtc_state->dma_addr) + i) = i & 0xff;
	}
...
static int rockchip_read_distro_logo(void *logo_addr, const char *bmp_name, int size)
{
	...
	if (!rockchip_get_bootdev_desc(&dev_desc) || !logo_addr)
		return -ENODEV;
...
#ifdef CONFIG_DRM_ROCKCHIP_VIDEO_FRAMEBUFFER
	screen_mode = SCREEN_MODE_CONSOLE_ACTIVE;
#endif
}
...
#ifdef CONFIG_DRM_ROCKCHIP_VIDEO_FRAMEBUFFER
	screen_mode = SCREEN_MODE_LOGO_ACTIVE;
#endif
...
{
	debug("(SS) rockchip_display_probe: called\n");
	...
	debug(" - initializing display buffer...\n");
	init_display_buffer(plat->base);

	debug(" - trying to find if routes exist in dtb...\n");
	...
	ofnode_for_each_subnode(node, route_node) {
		...
		if (!ofnode_is_enabled(node))
			continue;
		debug("      - getting connect handle...\n");
		phandle = ofnode_read_u32_default(node, "connect", -1);
		if (phandle < 0) {
			printf("Warn: can't find connect node's handle\n");
			continue;
		}
		debug("      - getting endpoint node...\n");
		ep_node = of_find_node_by_phandle(NULL, phandle);
		if (!ofnode_valid(np_to_ofnode(ep_node))) {
			printf("Warn: can't find endpoint node from phandle\n");
			continue;
		}
		debug("      - getting port node...\n");
		...
		debug("      - getting port parent node...\n");
	...
	debug(" - display route setup complete! getting baseparameter...\n");
	rockchip_get_baseparameter();
	
	debug(" - running display pre-init...\n");
	display_pre_init();

	debug(" - activating frame buffer...\n");
	...
	console_fbbase = plat->base; /* SS: so we can show console later */
	screen_mode = SCREEN_MODE_CONSOLE_ACTIVE;
	#endif

	debug(" - display bringup complete!\n");
	
	for(int i = 0; i < 1024; ++i) {
		debug("%02X ", ((unsigned char *)plat->base)[i]);
		if(i % 16 == 0) {
			debug("\n");
		}
	}
	...
	/*
	debug_binary_dump_grf("VOP register file SYS_CTRL after bringup", 
		(void *)0xfdd90000, (void *)0xfdd900ff);
	debug_binary_dump_grf("VOP register file DSC_SYS_CTRL after bringup", 
		(void *)0xfdd90200, (void *)0xfdd902ff);
	debug_binary_dump_grf("VOP register file OVERLAY after bringup", 
		(void *)0xfdd90600, (void *)0xfdd906ff);
	debug_binary_dump_grf("VOP register file POST0 after bringup", 
		(void *)0xfdd90c00, (void *)0xfdd90cff);
	debug_binary_dump_grf("VOP register file POST1 after bringup", 
		(void *)0xfdd90d00, (void *)0xfdd90dff);
	debug_binary_dump_grf("VOP register file POST2 after bringup", 
		(void *)0xfdd90e00, (void *)0xfdd90eff);
	debug_binary_dump_grf("VOP register file POST3 after bringup", 
		(void *)0xfdd90f00, (void *)0xfdd90fff);
	debug_binary_dump_grf("VOP register file CLUSTER0 after bringup", 
		(void *)0xfdd91000, (void *)0xfdd911ff);
	debug_binary_dump_grf("VOP register file CLUSTER1 after bringup", 
		(void *)0xfdd91200, (void *)0xfdd913ff);
	debug_binary_dump_grf("VOP register file CLUSTER2 after bringup", 
		(void *)0xfdd91400, (void *)0xfdd915ff);
	debug_binary_dump_grf("VOP register file CLUSTER3 after bringup", 
		(void *)0xfdd91600, (void *)0xfdd917ff);
	debug_binary_dump_grf("VOP register file ESMART0 after bringup", 
		(void *)0xfdd91800, (void *)0xfdd919ff);
	debug_binary_dump_grf("VOP register file ESMART1 after bringup", 
		(void *)0xfdd91a00, (void *)0xfdd91bff);
	debug_binary_dump_grf("VOP register file ESMART2 after bringup", 
		(void *)0xfdd91c00, (void *)0xfdd91dff);
	debug_binary_dump_grf("VOP register file ESMART3 after bringup", 
		(void *)0xfdd91e00, (void *)0xfdd91fff);
	debug_binary_dump_grf("VOP register file DSC_8K after bringup", 
		(void *)0xfdd94000, (void *)0xfdd940ff);
	debug_binary_dump_grf("VOP register file DSC_4K after bringup", 
		(void *)0xfdd94100, (void *)0xfdd941ff);
	*/
 ...
 */
static int rockchip_fdt_pack_reg(const void *fdt, void *buf, u64 *address, u64 *size,
				 int n)
{
	int i;
...
static int rockchip_fdt_update_reserved_memory(void *blob, char *name, u64 start, 
					       u64 size)
{
	...
	len = rockchip_fdt_pack_reg(blob, tmp, &start, &size, 1);
...
{
	debug("(SS) rockchip_display_fixup: called\n");
	...
	if (fdt_node_offset_by_compatible(blob, 0, "rockchip,drm-logo") >= 0) {
		...
		offset = rockchip_fdt_update_reserved_memory(blob, "rockchip,drm-logo",
							     (u64)memory_start,
							     (u64)get_display_size());
		if (offset < 0)
			printf("failed to reserve drm-loader-logo memory\n");

		offset = rockchip_fdt_update_reserved_memory(blob, "rockchip,drm-cubic-lut",
		...
		offset = rockchip_fdt_update_reserved_memory(blob, "rockchip,fb-logo",
							     (u64)memory_start,
							     MEMORY_POOL_SIZE);
		if (offset < 0)
...
#ifdef CONFIG_DRM_ROCKCHIP_VIDEO_FRAMEBUFFER
int rockchip_display_kill(void)
{
	struct display_state *s;
...
#ifdef CONFIG_DRM_ROCKCHIP_VIDEO_FRAMEBUFFER
static int do_rockchip_show_fbcon(cmd_tbl_t *cmdtp, int flag, int argc,
				  char *const argv[])
{
	_DEBUG_UART_ANNOUNCE;

	if (screen_mode != SCREEN_MODE_CONSOLE_ACTIVE)
		rockchip_show_fbbase(console_fbbase);
...
#ifdef CONFIG_DRM_ROCKCHIP_VIDEO_FRAMEBUFFER
U_BOOT_CMD(
	rockchip_show_fbcon, 1, 1, do_rockchip_show_fbcon,
	"show Rockchip framebuffer console (NEW!)",
	NULL
);
#endif
*/
